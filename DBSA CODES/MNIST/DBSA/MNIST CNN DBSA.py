...
# Define the AllCNN-C model for MNIST
class AllCNN_C_MNIST(nn.Module):
    def __init__(self):
        super(AllCNN_C_MNIST, self).__init__()

        self.features = nn.ModuleList([
            nn.Conv2d(1, 96, kernel_size=3, stride=1, padding=1),  # 1 channel for MNIST
            nn.BatchNorm2d(96),
            nn.ReLU(),
...
def enhanced_dynamic_batch_size_adjuster(B_t, L_t, G_t, avg_loss, avg_gradient, alpha=15.0, beta=1.5, kappa=50, min_batch=16, max_batch=512):
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs, initial_batch_size=512):
...
def main():
    ...
    transform_train = transforms.Compose([
        ...
        transforms.RandomCrop(28),  # Crop back to 28x28
    ...
    # Initial dataloaders (will be updated dynamically)
    ...
    # Plot 2: Batch Size over Epochs
    plt.figure(figsize=(10, 5))
    plt.plot(epochs_range, history['batch_size'], label='Batch Size', color='green')
    plt.xlabel('Epoch')
    plt.ylabel('Batch Size')
    plt.title('Dynamic Batch Size Oscillation over Epochs')
    plt.legend()
    plt.grid(True)
    plt.show()
